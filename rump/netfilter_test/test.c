...
main(int argc, char *argv[])
{
    ...
    iface = strdup(argc < 2 ? "xenif0" : argv[1]);
    if (iface == NULL)
...
/*
    ...
    if( !accept( sockfd, (struct sockaddr *)NULL, NULL ) )
...
open_dev()
{
    ...
    /* Open the bpf device */
    snprintf(dev, sizeof(dev), "/dev/bpf");
...
*/
    ...
    // block ip address 10.30.30.99
    struct bpf_insn insns[] = {
        BPF_STMT(BPF_LD+BPF_H+BPF_ABS, 12),
        BPF_JUMP(BPF_JMP+BPF_JEQ+BPF_K, ETHERTYPE_IP, 0, 3),
        BPF_STMT(BPF_LD+BPF_W+BPF_ABS, 26),
        BPF_JUMP(BPF_JMP+BPF_JEQ+BPF_K, 0x0a1e1e63, 1, 0),
        BPF_STMT(BPF_RET+BPF_K, (u_int)-1),
        BPF_STMT(BPF_RET+BPF_K, 0),
    };
